...
import { ICreateIngredientsOfFood, IIngredientsOfFoodDto, IUpdateIngredientsOfFood } from "@/features/ingredientsOfFoods/types/IngredientOfFood";
...
export async function GetAllIngredientByFoodId(foodId: string): Promise<IIngredientsOfFoodDto[]> {
    ...
    const { data } = await api.get<IIngredientsOfFoodDto[]>(`/IngredientsOfFood/${foodId}`)
...
export async function CreateIngredientOfFood(dto: ICreateIngredientsOfFood): Promise<IIngredientsOfFoodDto> {
    ...
    const { data } = await api.post<IIngredientsOfFoodDto>("/IngredientsOfFood", dto);
...
export async function UpdateIngredientOfFood(dto: IUpdateIngredientsOfFood): Promise<void> {
    ...
    await api.put(`/IngredientsOfFood/${dto.id}`, dto);
...
export async function DeleteIngredientOfFood(id: string): Promise<void> {
    ...
    await api.delete(`/IngredientsOfFood/${id}`);
