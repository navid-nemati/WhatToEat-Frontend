...
export default function CreateOrUpdateIOF({
    ...
    const { mutate: updateMutate, isPending: updateIsPending, isError: updateIsError, error: updateError } = UseUpdateIngredientOfFood()
    const { mutate: createMutate, isPending: createIsPending, isError: createIsError, error: createError } = UseCreateIngredientOfFood()

    const isPending = isEditingMode === "Edit" ? updateIsPending : createIsPending;
    const isError = isEditingMode === "Edit" ? updateIsError : createIsError;
    const error = isEditingMode === "Edit" ? updateError : createError;

    const parsedError = isError ? parseApiError(error) : null;

    const { register, handleSubmit, formState: { errors }, reset } = useForm<UpdateIngredientFormData>({
        resolver: zodResolver(UpdateIngredientSchema),
        defaultValues: { name: "" }
    })
    ...
    useEffect(() => {
        if (isEditingMode !== "close") {
            reset({ name: defaultIngredientValue || "" });
            setSelectedIngredientId("");
            setIngredientIdError("");
        }
    }, [isEditingMode, defaultIngredientValue, reset]);
    ...
    const onSubmit = (data: UpdateIngredientFormData) => {
        ...
        if (isEditingMode === "Create") {
            ...
            updateMutate({ ...payload, id: ingredientOfFoodId }, {
                onSuccess: () => {
                    reset()
                    setSelectedIngredientId("")
                    setIsEditingMode("close")
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
            <div className="bg-white rounded-xl w-full max-w-md shadow-2xl p-6 md:p-8 flex flex-col gap-4 max-h-[90vh] overflow-y-auto">
                <span className="text-xl font-bold text-gray-800 border-b pb-3">
                    {isEditingMode === "Create" ? "افزودن ماده اولیه" : "ویرایش ماده اولیه"}
                </span>

                <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
                    
                    <SelectIngredient key={isEditingMode + ingredientOfFoodId} onSelect={handleSelection} />

                    {ingredientIdError && (
                        <p className="text-red-500 text-sm text-center -mt-2">{ingredientIdError}</p>
                    )}

                    <div className="flex flex-col gap-2">
                        <label className="text-sm font-medium text-gray-600">مقدار مورد نیاز</label>
                        <TextField
                            fullWidth
                            size="small"
                            placeholder="مثال: ۲۰۰ گرم یا 2 قاشق غذاخوری"
                            variant="outlined"
                            {...register("name")}
